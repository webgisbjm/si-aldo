...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.density.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.densities.update", [$density->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kelurahans') ? 'is-invalid' : '' }}" name="kelurahans_id" id="kelurahans_id" required>
                    @foreach($kelurahans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kelurahans_id') ? old('kelurahans_id') : $density->kelurahans->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('area') ? 'is-invalid' : '' }}" type="number" name="area" id="area" value="{{ old('area', $density->area) }}" step="0.01" required>
                @if($errors->has('area'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('population') ? 'is-invalid' : '' }}" type="number" name="population" id="population" value="{{ old('population', $density->population) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('density') ? 'is-invalid' : '' }}" type="number" name="density" id="density" value="{{ old('density', $density->density) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" name="year" id="year" required>
                    ...
                    @foreach(App\Models\Density::YEAR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('year', $density->year) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('year'))
