    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-pages.update", [$contentPage->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no') ? 'is-invalid' : '' }}" type="text" name="no" id="no" value="{{ old('no', $contentPage->no) }}" step="1">
                @if($errors->has('no'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="number" name="year" id="year" value="{{ old('year', $contentPage->year) }}" step="0.01" min="1945" max="2024">
                @if($errors->has('year'))
            ...
            <div class="form-group">
              ...
              <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="content_categories_id" id="content_categories_id" required>
                  @foreach($categories as $id => $entry)
                      <option value="{{ $id }}" {{ (old('content_categories_id') ? old('content_categories_id') : $contentPage->categories->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                  @endforeach
              </select>
